...
<template>
  <div class="mx-auto max-w-md space-y-8">
    <header class="space-y-3 text-center">
      ...
      <h1 class="text-3xl font-semibold text-slate-100">Sign in</h1>
      <p class="text-sm text-slate-400">Use your email and password.</p>
    </header>
    ...
    <form class="space-y-4" @submit.prevent="submit">
      ...
      <div class="grid gap-2">
        ...
        <input v-model="password" type="password" required class="rounded-2xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-slate-100" />
      </div>
      <p v-if="error" class="rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-2 text-xs text-red-200">{{ error }}</p>
      <button type="submit" class="w-full rounded-full bg-gradient-to-br from-brand to-brand-accent px-5 py-2 text-sm font-medium text-white shadow-glow">Sign in</button>
      ...
      <div class="text-center text-xs text-slate-400">
        No account?
        <RouterLink :to="{ name: 'sign-up' }" class="text-brand-accent">Sign up</RouterLink>
      </div>
    </form>
  </div>
</template>
